import numpy as np
from scranpy.dimensionality_reduction import RunPcaArgs, run_pca
...
def test_run_pca(mock_data):
    x = mock_data.x * 20
    res = run_pca(x, RunPcaArgs(rank=10))

    mres = run_pca(
        x, RunPcaArgs(rank=10, block=mock_data.block, block_method="project")
    )
