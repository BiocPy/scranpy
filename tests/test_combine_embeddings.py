import numpy as np
from scranpy.dimensionality_reduction import combine_embeddings, CombineEmbeddingsOptions
...
def test_combine_embeddings_weighted(mock_data):
    ...
    res = combine_embeddings([x1, x2, x3], CombineEmbeddingsOptions(weights = [1.5, 0, 0.5]))
    assert res.shape[0] == 200
    ...
    empty = combine_embeddings([x1, x2, x3], CombineEmbeddingsOptions(weights = [0, 0, 0]))
