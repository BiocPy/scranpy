        ...
        if isinstance(expr.value, ast.Name):
            if expr.value.id in translations:
                expr.value = ast.Attribute(
                    value=ast.Name("scranpy"), attr=translations[expr.value.id]
                )
        else:
            trawl(expr.value)
...
def capture(expr):
    return ast.Expr(
        ast.Call(
            func=ast.Attribute(attr="append", value=ast.Name("__commands")),
...
for expr in fun.body:
    if not capturing:
        if isinstance(expr, ast.Assign):
            if (
                isinstance(expr.targets[0], ast.Name)
                and expr.targets[0].id == "results"
            ):
                capturing = True
                new_body.append(
                    ast.parse(
...
new_body.append(
    ast.Return(
        value=ast.Call(
            func=ast.Attribute(attr="join", value=ast.Constant("\n")),
...
print(
    "# DO NOT MODIFY THIS FILE! This is automatically generated by '"
    + sys.argv[0]
    + "'\n# from the source file '"
    + sys.argv[1]
    + "', modify that instead and rerun the script.\n"
)
